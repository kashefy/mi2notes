...
A further use-case is one where pairwise distance is not measured explicitly. The data representation can already be in a pairwise fashion as a result of an algroithm. Example: Sequence alignment proceducres and graph-similarity measures produce pairwise representations.\\

We will opt for the \emph{squared Eucldiean distance} for our pairwise clustering algorithm.
...
\notesonly{
We've seen that Pairwise Clustering based on squared Euclidean distance boils down to K-means. Pairwise clustering can therefore be considered a generelization of K-means. One can also think of K-means as a special case of pairwise clustering.}
\slidesonly{
