...
\begin{frame} 
    ...
    \end{center}
    \begin{center}
    Key measures from information theory which play a role in Infomax ICA
    \end{center}
\end{frame}
}
...
Imagine flipping a biased coin which reveals \text{tails} most of the time. 
We will no longer be ``surprised'', if we flip the coin again and see it show \text{tails} \textbf{again}.\\
...
\svspace{-5mm}
\begin{center}
	\includegraphics[width=0.6\textwidth]{img/aldisain_Plasma_TV}
\end{center}
}
...
\slidesonly{
	...
	\includegraphics<5>[width=0.35\textwidth]{img/Wooden-TV_on}
	\includegraphics<6>[width=0.35\textwidth]{img/Wooden-TV_noise}
\end{center}
...
\notesonly{- No, this is not necessary. An input-output relationship is just a clear and intuitive way to describe dependency betweem the two variables.}
...
\end{equation}}such facotrization can be found by minimizing 
...
In spite of this being an apparent approach for blind source separation, it requires a paramtereized density estimate which would make this approach computationally costly.
}
\only<3>{
\slidesonly{
\begin{center}
	\includegraphics[width=0.4\textwidth]{img/meme_anotherway}
\end{center}
}
...
\pause
\notesonly{
This is referred to as t}\slidesonly{T}he mutual information $I(X,Y)$ between the random variables $X$ and $Y$:
